...
public class OnboardStateService {
    ...
    private static final int TOTAL_ONBOARD_STEPS = 8; // 총 온보딩 단계 수
    ...
    public void updateProgress(UUID userId, Integer completedStep) {
        ...
        try {
            ...
            double progressPercentage = (completedCount / (double) TOTAL_ONBOARD_STEPS) * 100;
            
            // 진행률 데이터 업데이트
            progress.put("completedSteps", completedSteps);
            progress.put("currentStep", completedStep + 1 > TOTAL_ONBOARD_STEPS ? TOTAL_ONBOARD_STEPS : completedStep + 1);
            ...
            log.info("Progress updated for user {}: {}/{} steps completed ({}%)", 
                userId, completedCount, TOTAL_ONBOARD_STEPS, Math.round(progressPercentage));
     ...
     * 온보딩 상태 조회
     */
    public com.studymate.domain.onboard.dto.response.OnboardStatusResponse getOnboardStatus(UUID userId) {
        ...
        if (progressData.isPresent()) {
            ...
            return com.studymate.domain.onboard.dto.response.OnboardStatusResponse.builder()
                    .currentStep(currentStep)
                    .totalSteps(TOTAL_ONBOARD_STEPS)
                    .progressPercentage(progressPercentage)
                    .completed(currentStep >= TOTAL_ONBOARD_STEPS)
                    .build();
        }
     ...
     * 온보딩 데이터 삭제
     */
    public void clearOnboardData(UUID userId) {
     ...
     * 온보딩 완료 처리
     */
    public void completeOnboard(UUID userId) {
        String completedKey = "onboard:" + userId.toString() + ":completed";
        redisTemplate.opsForValue().set(completedKey, "true", DEFAULT_TTL);
    }

    /**
     * 온보딩 완료 여부 확인
     */
    public boolean isOnboardCompleted(UUID userId) {
        String completedKey = "onboard:" + userId.toString() + ":completed";
        String value = redisTemplate.opsForValue().get(completedKey);
