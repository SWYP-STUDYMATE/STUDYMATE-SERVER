...
import com.studymate.domain.onboard.service.OnboardService;
...
public class OnboardController {
    ...
    private final OnboardService onboardingService;

    @GetMapping("/data")
    public OnboardDataResponse getOnboardingData(@AuthenticationPrincipal CustomUserDetails principal) {
    ...
    public ResponseEntity<ApiResponse<OnboardProgressResponse>> saveOnboardingStep(
            @AuthenticationPrincipal CustomUserDetails principal,
            @Parameter(description = "저장할 단계 번호 (1-7)", required = true)
            @PathVariable Integer stepNumber,
            @Valid @RequestBody OnboardStepRequest request) {
        
        UUID userId = principal.getUuid();
        OnboardProgressResponse progress = onboardingService.saveOnboardingStep(userId, stepNumber, request);
    ...
    public ResponseEntity<ApiResponse<OnboardProgressResponse>> skipOnboardingStep(
            @AuthenticationPrincipal CustomUserDetails principal,
        ...
        UUID userId = principal.getUuid();
        OnboardProgressResponse progress = onboardingService.skipOnboardingStep(userId, stepNumber, reason);
